...
class Solution:
    def maximumScore(self, nums: List[int], k: int) -> int:
        #can't repeat subarray
        #prime score of n = num of prime factors of n
        # Order of mult doesn't matter
        # fine prime scores: n + sqrt(n)
        # bru force: find max element, count
        # subarray where it has max score, which is 0(n)
        # total complesity: 0(n^2)

        # Preprocessing? Prefix/suffix?
        N = len(nums)
        MOD = 10**9 + 7
        res = 1
